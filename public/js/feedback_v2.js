...
function createReviewCard(review) {
    ...
    const isLiked = !!review.user_liked;
...
async function loadComments(reviewId) {
    try {
        ...
        result.data.forEach(comment => {
            ...
            if (isOwner) {
                actions += `
                    <button class="comment-action-btn edit-btn" onclick="editComment(${comment.id})" title="Редактировать">
...
async function deleteComment(commentId) {
    ...
    try {
        const response = await fetch(`${API_URL}/review-comments/${commentId}`, {
            method: 'DELETE',
            headers: { 'x-user-id': currentUser.id }
        });
...
async function submitEditComment() {
    ...
    try {
        const response = await fetch(`${API_URL}/review-comments/${editingCommentId}`, {
            method: 'PATCH',
            headers: {
                'Content-Type': 'application/json',
                'x-user-id': currentUser.id
