...
const ADMIN_IDS = ['299696306', 'YOUR_ID_2']; // Замените на реальные ID
...
function insertBooking(connection, data) {
    const sql = 'INSERT INTO bookings (user_id, username, date, slot_time, end_time, floor, comment) VALUES (?,?,?,?,?,?,?)';
    const params = [data.user_id, data.username, data.date, data.slot_time, data.end_time, data.floor, data.comment];
    ...
    pool.getConnection(async (err, connection) => {
        ...
        try {
            ...
            if (end_time < slot_time) {
                ...
                await insertBooking(connection, { user_id, username, date, slot_time, end_time: "24:00", floor, comment: comment + " (Часть 1)" });
                await insertBooking(connection, { user_id, username, date: nextDate, slot_time: "00:00", end_time, floor, comment: comment + " (Часть 2)" });
                ...
                await insertBooking(connection, { user_id, username, date, slot_time, end_time, floor, comment });
